...
## 2026-09-23：V1.3.1 API 边界加固

主要成果：

- 新增空白用户名、空白岗位 ID 和错误字段类型反例，确认 FastAPI/Pydantic 在分析前返回 422。
- 新增岗位文件与候选人证据文件缺失反例，确认接口返回 503 且不泄露本机路径。
- 现有 V1.3 接口实现直接通过全部新增反例，因此没有修改业务实现或响应结构。
- README 明确服务只应监听 `127.0.0.1`，没有身份验证，不能用于真实候选人数据或公网部署。
