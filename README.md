...
- V1.4 增加项目主张与代码、测试、演示证据的对应说明，以及可核对的技术决策和项目讲解材料；没有增加业务接口或模型能力。
...
V1.4 再次在全新 Python 3.13.5 虚拟环境中安装现有依赖并通过 168 项测试。重新运行的 26 个固定案例与保存的脱敏报告完全一致；本机真实 HTTP 验收的三个正常请求返回 200，额外文件字段返回 422，分析响应与保存的脱敏示例逐字段一致。17 份受版本控制 JSON 保持不变。求职表述的项目证据与局限见 [`docs/PROJECT_EVIDENCE.md`](docs/PROJECT_EVIDENCE.md)；个人贡献仍需根据本人实际过程核对。
