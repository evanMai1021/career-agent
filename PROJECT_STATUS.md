...
- 后续功能继续按新版本需求开发；不在第一版范围内临时加入 RAG、多 Agent 或云部署。
...
1. 当前 V0.8.1 修改位于 `codex/v0.8.1-nonempty-text-validation` 分支，待用户复查后再决定提交和推送。
2. 下一项新功能优先考虑结构化岗位目标读取与岗位相关建议，但应先确定数据来源和最小 Schema，不直接扩展到 RAG。
...
- 公开分支 `main` 与 GitHub 远程已核对为同一提交 `0d293cb`；V0.8.1 当前仍在独立开发分支中。
